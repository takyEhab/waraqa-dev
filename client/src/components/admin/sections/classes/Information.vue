  ...
  methods: {
    ...
    rescheduleReq(select) {
      ...
      if (select == "acceptOfTeacher") {
        data.startingDate = moment(this.data[0].teacherReschedule).format(
          "YYYY-MM-DD HH:mm"
        );
        ...
        data.startingDate = moment(this.data[0].studentReschedule).format(
          "YYYY-MM-DD HH:mm"
        );
      ...
      let url = `http://localhost:3300/api/v1/admin/classes/path4/${this.classID}`;
      axios
        .post(url, data)
        .then((res) => {
          if (!res.data.success) {
    ...
    deleteData() {
      ...
      if (window.confirm("Are you sure you want to delete this Class?")) {
        axios
          .delete(
            `http://localhost:3300/api/v1/admin/classes/path6/${this.classID}`
          )
          .then((res) => {
            if (!res.data.success) {
              return (this.alerts.error = res.data.msg);
