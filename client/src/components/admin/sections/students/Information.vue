      ...
      <div class="mt-5">
        ...
        <div class="row flex-wrap">
          ...
          <div
            class="mt-3 col-12 col-md-6 col-lg-4 d-flex justify-content-between"
          >
            <small class="info-title">Time Zone :</small>
            <span class="info-content">{{
      ...
      <div class="b-color-0 box-shadow-style mt-5 p-3">
        <div class="d-flex justify-content-between align-items-center">
          <div class="col-11 col-md-3 mb-1">
            <span class="d-block f-color-2">Periodic evaluations list :</span>
          </div>
          <div
            class="col-1 circle-Add"
            type="button"
            data-bs-toggle="modal"
        ...
        <!-- Table -->
        <div
          v-if="periodicEvaluations.length"
        ...
        >
          <div style="min-width: 62em">
            <div class="px-2 table-colums f-color-1 d-flex mb-4">
              <small class="col-2">Meeting Date</small>
              <small class="col-2">Teacher</small>
              <small class="col-4">Teacher performance</small>
              <small class="col-4">Student performance</small>
            </div>
            <div
              v-for="row in periodicEvaluations"
              :key="row.id"
              class="table-row py-3"
            >
              <div class="px-2 d-flex f-color-3">
                <small class="col-2">{{
                  moment(row.meetingDate).format("ddd, D MMM YYYY")
                }}</small>
                <small class="col-2">{{ row.teacherName }}</small>
                <small class="col-4">{{ row.teacherPerformance }}</small>
                <small class="col-4">{{ row.studentPerformance }}</small>
              </div>
            </div>
            <!-- Alerts -->
            <div class="mt-4">
...
import Edit from "@/components/admin/sections/students/Edit";
import AddEvaluation from "@/components/admin/sections/students/AddEvaluation";
export default {
  components: { Edit, AddEvaluation },
  data() {
    return {
      displayData: false,
      loadingBtn: false,
    ...
    getPeriodicEvaluation() {
      ...
      axios
        ...
        .then((res) => {
          ...
          this.alerts.error = null;
          this.periodicEvaluations = res.data.rows;
        })
        .catch(() => {
          console.log("periodic Info/Error catched");
        });
    },
    deleteUser() {
      this.alerts.error = null;
