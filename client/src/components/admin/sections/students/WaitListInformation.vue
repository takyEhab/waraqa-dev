...
export default {
  ...
  methods: {
    getUserData() {
      let url = `http://localhost:3300/api/v1/admin/students/path8/${this.userID}`;
      axios
        .get(url)
        .then((res) => {
          if (!res.data.success) {
    ...
    updateUserData() {
      ...
      let url = `http://localhost:3300/api/v1/admin/students/path9/${this.userID}`;
      axios
        .post(url, this.data)
        .then((res) => {
          if (!res.data.success) {
    ...
    deleteUser() {
      ...
      if (window.confirm("Are you sure you want to delete this Student?")) {
        axios
          .delete(
            `http://localhost:3300/api/v1/admin/students/path10/${this.userID}`
          )
          .then((res) => {
            if (!res.data.success) {
              return (this.alerts.error = res.data.msg);
